...
for i in range(20):
    for j in range(20):
        if i + 4 < 20:
            pro = max(calcPro(i, j, 1, 0), pro)  # equivalent of left and right
        if j + 4 < 20:
            pro = max(calcPro(i, j, 0, 1), pro)  # equivalent of up and down
            if i + 4 < 20:
                pro = max(calcPro(i, j, 1, 1), pro)  # diag 1
            if i - 4 > -1:
                pro = max(calcPro(i, j, -1, 1), pro)  # diag 2
